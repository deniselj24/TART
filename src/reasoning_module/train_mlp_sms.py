...
def train_step(
    ...
    #print("ys", ys.shape)
    # convert to binary predictions for the SMS task 
    #print("output", output)
...
def train(model, args):
    ...
    data_sampler = get_data_sampler(args.training.data, n_dims=n_dims, split="train")
    ...
    for i in pbar:
        ...
        if i % args.wandb.log_every_steps == 0 and not args.test_run:
            ...
            with torch.no_grad():
                # test accuracy 
                data_sampler = get_data_sampler(args.training.data, n_dims=n_dims, split="test")
                xs, ys = data_sampler.sample_xs(
                    curriculum.n_points,
                    batch_size,
                    curriculum.n_dims_truncated,
                ...
                output = model(xs, ys)
                output = output.cuda()
                loss = task.get_training_metric()(output, ys)
                #preds = torch.argmax(output, dim=-1) #multiclass
                #preds = (F.sigmoid(output) >= 0.5)
...
if __name__ == "__main__":
    ...
    assert args.model.family in ["mlp", "logistic_regression"]
